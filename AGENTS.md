...
| `_data/themes.yml` | The themes the preview picker offers, and the face each name is set in. A new theme needs an entry here as well as a file in `css/themes/` |
| `css/theme-switcher.css` | Styles and font imports for the preview-only theme picker. Linked from preview builds only |
...
- **Themes**: `site_theme` in `_config.yml` picks a file from `css/themes/`, loaded after `main.css`. A theme overrides tokens only, never structure, and must define **both** its light values on `:root` and its dark values in its own `prefers-color-scheme: dark` block — `main.css` has a dark block earlier in source order, so a light-only theme would leak into dark mode. Preview builds add a runtime switcher; production loads exactly one theme and no script. Adding a theme means two edits: the file in `css/themes/`, and an entry in `_data/themes.yml` (`name`, `label`, the `font` its label is set in, and the `google` family to load for it) so the picker lists it.
